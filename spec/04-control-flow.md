...
- **Loop labels**: No way to break out of nested loops. Labeled loops (`(loop :outer ...)` / `(break :outer)`) would solve this.
- **`when` as standalone guard**: No standalone `(when condition body)` form (like Ruby's). Must use `(if condition then body)`.
- **Tail call optimization**: Not implemented. Deep recursion (e.g., `fib(10000)`) will stack overflow. TCO would enable idiomatic recursive algorithms.
