...
| Pattern matching | Experimental | `spec/12-patterns.md` | Array and Gene destructuring plus `case/when` exist. Enum ADT `case` patterns support qualified and unambiguous bare variants, declaration-order payload binding, ignored `_` payload binders, unit variants, diagnostics for ambiguity/arity/unknown variants, and exhaustiveness diagnostics for statically known enum cases. The standalone `(match ...)` expression is removed. | Pattern coverage exists in `tests/integration/test_pattern_matching.nim`, `tests/integration/test_case.nim`, and `testsuite/12-patterns/`. | Tested subset is documented; nested patterns, guards, map destructuring, broad function-parameter patterns, or/as patterns, and broad non-enum arity diagnostics remain open. | Use the documented tested subset for durable code; use `(var pattern value)` for binding and `(case ...)` for branching; treat expansion beyond it as experimental/future. |
