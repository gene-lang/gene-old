...
| Aspect-oriented programming (AOP) | Experimental | `docs/proposals/future/aop.md` | Current audited runtime supports `(aspect ...)`, class method `.apply`, explicit `.apply-fn` wrappers, per-interception toggles, chaining, callable advice, and narrow inline lexical capture. | Focused AOP fixtures cover the audited surface across class-method wrappers, standalone wrappers, controls, boundaries, callable advice, lexical capture, and macro-boundary behavior. | Outside stable core and outside Beta guarantees; standalone `.apply-fn` keyword calls, macro-transparent wrappers, stale APIs (`fn_aspect`, `.apply_in_place`), broader join points, async/reset/priority hardening, and durable usage guidance are deferred. | Use only for exploration and audit-backed maintainer experiments; avoid depending on it for durable code. |
...
- advanced classes, adapters, experimental AOP, or broad object-model guarantees
