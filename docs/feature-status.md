...
| Gradual type system | Beta | `docs/type-system-mvp.md`, `docs/how-types-work.md`, `spec/02-types.md` | Compile-time checking, runtime descriptor validation, typed arguments, locals, returns, typed properties, and enum type annotations are partially delivered. | Type checker and typed tests exist in `tests/test_type_checker.nim`, `testsuite/02-types/`, and typed function tests. | Canonical descriptor pipeline, negative-path coverage, flow typing breadth, generic classes, and full enum constructor/type enforcement remain incomplete. | Use explicit annotations at boundaries; expect beta-level churn. |
| Enum ADT declarations | Beta | `docs/enum-adt-unification.md`, `spec/02-types.md` | `enum` is the canonical public ADT declaration model. Generic enum heads, unit variants, payload variants, ordered payload field names, optional field type descriptors, base-name canonicalization, and targeted malformed-declaration diagnostics are implemented for the S01 boundary. | Declaration-contract coverage exists in `testsuite/02-types/types/16_enum_declaration_contract.gene`, enum regressions, type-checker tests, and GIR metadata tests. | Constructor enforcement, Result/Option cleanup, enum pattern matching, import identity, persistence, and final examples are downstream M002 work. | Use `enum` for new sum-type declarations; do not use legacy Gene-expression ADT declarations as a public model. |
...
| Pattern matching | Experimental | `spec/12-patterns.md` | Array and Gene destructuring plus simple `case/when` exist; enum variant matching and `?` are explicitly downstream/experimental. | Pattern coverage exists in `tests/integration/test_pattern_matching.nim`, `tests/integration/test_case.nim`, and `testsuite/12-patterns/`. | Tested subset is documented; enum variant patterns, exhaustiveness, nested patterns, guards, map destructuring, broad function-parameter patterns, match/or/as patterns, and arity diagnostics remain open. | Use only the documented tested subset for durable code. |
| GIR bytecode/cache format | Beta | `docs/gir.md`, `docs/gir-benchmarks.md` | GIR serialization and caching exist with GIR version, compiler version, Value ABI, instruction ABI, and source-hash checks plus CLI support. | GIR coverage exists in `tests/integration/test_cli_gir.nim`, `test_cli_run.nim`, and stable-core stress tests. | The format is still versioned for local VM compatibility rather than a long-term portable bytecode contract; enum identity persistence is still being completed. | Useful for cold-start and cache workflows; validate with source when debugging. |
...
- enum ADT behavior beyond the current declaration-contract boundary
